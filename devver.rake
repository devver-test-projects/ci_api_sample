...
namespace :devver do
  ...
  namespace :spec do
    ...
    desc "Runs all routing specs on Devver"
    task :routing do
      devvertest('spec/routing/**/*_spec.rb')
    end
...
def devvertest(pattern)
  reload = get_env_var('reload')=='true'
  #default sync to true
  sync = true 
  sync = false if get_env_var('sync')=='false'
  cache = get_env_var('cache')=='true' 
  display_output = get_env_var('output')=='true'
  db = get_env_var('db')=='true'
  ...
  if run_excluded!='only'
    command = "#{APP_CALL} #{'--display-output' if display_output} #{'--reload' if reload} #{'--nosync' if !sync} #{'--db' if db} #{'--cache' if cache} #{files.join(' ')}"
    puts command
    results = system(command)
    raise RuntimeError.new("Command failed with status (1)") unless results
  end
